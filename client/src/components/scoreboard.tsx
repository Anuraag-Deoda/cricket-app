...
import type { Match, Player, Team, Innings, MatchSituation } from "@shared/types";
import {
  Table,
  TableBody,
  TableCell,
...
export default function Scoreboard({ match, setMatch, onBowlerChange, isSimulating }: { match: Match, setMatch: (match: Match) => void, onBowlerChange: (bowlerId: number) => void, isSimulating?: boolean }) {
  ...
  const showBowlerSelection = currentInnings.currentBowler === '' && match.status === 'inprogress' && currentInnings.overs < match.oversPerInnings;
